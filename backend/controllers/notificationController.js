...
exports.UpdateSingleNotification = async (req, res) => {
    try {
        ...
        if (notification.user !== req.user) return res.json({ status: 404, msg: 'You are not authorized to process this data' })
...
exports.DeleteNotification = async (req, res) => {
    try {
        ...
        return res.json({ status: 200, msg: 'Notification deleted successfully' })
    } catch (error) {
        res.json({ status: 500, msg: error.message })
    }
}
