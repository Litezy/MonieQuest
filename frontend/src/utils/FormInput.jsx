...
const FormInput = ({ formtype = 'text', border = true, label, type = 'text', value, name, placeholder, className, onChange, onKeyUp, clas }) => {
  return (
    <div className='flex flex-col gap-2'>
      ...
      {formtype === 'textarea' && <textarea type={type} className={`outline-none focus-within:outline-none focus:outline-none focus:ring-0 focus:border-gray-400 focus:border bg-transparent lg:text-sm text-base w-full h-32 rounded-xl py-3 px-4 resize-none scrollHide ${className}`} placeholder={placeholder} value={value} onChange={onChange} ></textarea>}
    </div>
  )
}
